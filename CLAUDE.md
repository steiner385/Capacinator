...
All environment variables are centralized in `src/server/config/env.ts`. This module:
- Provides TypeScript types for all configuration
- Validates required variables at startup
- Fails fast with clear error messages for invalid config
- Exports a typed `env` object for use throughout the server
...
console.log(env.database.filename);  // string
console.log(env.auth.jwtSecret);     // string
console.log(env.audit.enabled);      // boolean
```

**Configuration Sections:**
- `env.server` - NODE_ENV, port, isDevelopment, isProduction, isTest, isE2E
- `env.database` - filename, databaseUrl, backupEnabled, backupRetentionDays
- `env.auth` - jwtSecret, jwtExpiresIn, jwtRefreshExpiresIn, bcryptRounds
- `env.fileUpload` - maxFileSize, uploadDir
- `env.email` - smtp config (host, port, user, pass, from), appUrl
- `env.logging` - level, format, directory, maxFileSize, maxFiles
- `env.audit` - enabled, maxHistoryEntries, retentionDays, sensitiveFields, enabledTables
- `env.app` - autoSaveInterval, enableAutoUpdate

### Required
| Variable | Description | Default |
|----------|-------------|---------|
| NODE_ENV | Environment (development/production/test/e2e) | development |
...
| DB_FILENAME | SQLite database file | capacinator.db |

### Optional
| Variable | Description | Default |
|----------|-------------|---------|
| LOG_LEVEL | Logging level (error/warn/info/http/debug) | info |
| LOG_FORMAT | Log format (human/json) | human (dev), json (prod) |
| AUDIT_ENABLED | Enable audit logging | false (auto-enabled in e2e) |
| DB_BACKUP_ENABLED | Enable automatic backups | true |
| DB_BACKUP_INTERVAL | Backup frequency | daily |
| DB_BACKUP_RETENTION_DAYS | Days to keep backups | 30 |
