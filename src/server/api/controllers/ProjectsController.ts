...
export class ProjectsController extends BaseController {
  ...
  private async inheritProjectPhases(projectId: string, projectTypeId: string): Promise<void> {
    try {
      ...
      logger.info(`Successfully inherited ${timelineEntries.length} template phases for project ${projectId} from project type ${projectTypeId}`);
      
    } catch (error) {
      // Log the error but don't fail project creation
      logger.error(`Failed to inherit template phases for project ${projectId}`, error as Error, { projectId, projectTypeId });
