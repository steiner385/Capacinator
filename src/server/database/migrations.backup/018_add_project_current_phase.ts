...
export async function up(knex: Knex): Promise<void> {
  console.log('Adding current_phase_id to projects table...');
  
  // First, drop all views to avoid conflicts
  ...
  await knex.schema.alterTable('projects', (table) => {
    // Add current phase ID that references the project_phases table
    table.uuid('current_phase_id').nullable().references('id').inTable('project_phases').onDelete('SET NULL');
  ...
  await knex.raw(`
    ...
    LEFT JOIN project_phases current_phase ON p.current_phase_id = current_phase.id
  `);
  ...
  // Recreate all the other views that we dropped
  // Run the view creation from migration 015 but update project_health_view
  await knex.raw(`
  ...
  console.log('✅ Added current_phase_id to projects table');
}
...
export async function down(knex: Knex): Promise<void> {
  ...
  console.log('✅ Removed current_phase_id from projects table');
}