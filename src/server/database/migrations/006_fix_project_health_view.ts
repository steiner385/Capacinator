...
export async function up(knex: Knex): Promise<void> {
  console.log('Fixing project_health_view with correct health_status column...');

  // Drop the existing view
  await knex.raw('DROP VIEW IF EXISTS project_health_view');

  // Create the corrected project_health_view with health_status column
  await knex.raw(`
  ...
  console.log('✅ Fixed project_health_view with correct health_status column');
}

export async function down(knex: Knex): Promise<void> {
  console.log('Reverting project_health_view fix...');
  
  await knex.raw('DROP VIEW IF EXISTS project_health_view');
  
  console.log('✅ Reverted project_health_view');
}