...
export async function up(knex: Knex): Promise<void> {
  ...
  // 4. Create user_permissions table (user-specific overrides)
  await knex.schema.createTable('user_permissions', (table) => {
    ...
    table.uuid('granted_by').references('id').inTable('people').onDelete('SET NULL');
  ...
  await knex.schema.table('people', (table) => {
    table.uuid('user_role_id').references('id').inTable('user_roles').onDelete('SET NULL');
    table.boolean('is_system_admin').defaultTo(false);
    table.boolean('is_active').defaultTo(true);
