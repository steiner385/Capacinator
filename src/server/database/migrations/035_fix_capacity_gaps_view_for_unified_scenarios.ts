...
export async function up(knex: Knex): Promise<void> {
  console.log('Updating capacity_gaps_view to use unified scenario architecture...');

  // Drop the existing capacity_gaps_view
  await knex.raw('DROP VIEW IF EXISTS capacity_gaps_view');

  // Create updated capacity_gaps_view that uses effective_project_assignments
  await knex.raw(`
    ...
    role_demand AS (
      -- Calculate demand per role using effective assignments (unified scenario architecture)
      SELECT 
        r.id as role_id,
        -- Calculate demand from effective assignments (includes base + scenario assignments)
        COALESCE(SUM(
          epa.allocation_percentage / 100.0
        ), 0) as total_demand_fte,
        -- Calculate demand in hours per day
        COALESCE(SUM(
          (epa.allocation_percentage / 100.0) * 8.0
        ), 0) as total_demand_hours
      FROM roles r
      LEFT JOIN effective_project_assignments epa ON r.id = epa.role_id
      LEFT JOIN scenarios s ON epa.scenario_id = s.id
      -- Include base assignments (scenario_id is null) OR active scenario assignments
  ...
  console.log('✅ Updated capacity_gaps_view to use unified scenario architecture');

  // Also update the person_utilization_view to use effective assignments
  ...
  await knex.raw(`
    ...
    SELECT 
      ...
      -- Calculate total allocation from effective assignments (unified scenario architecture)
      COALESCE(SUM(
        CASE 
          -- Include base assignments (no scenario) OR active scenario assignments
          WHEN (s.status = 'active' OR epa.scenario_id IS NULL)
          THEN epa.allocation_percentage 
          ELSE 0 
        END
      ), 0) AS total_allocation_percentage,
      -- Count active projects from effective assignments
      COUNT(DISTINCT CASE 
        WHEN (s.status = 'active' OR epa.scenario_id IS NULL)
        THEN epa.project_id 
    ...
    LEFT JOIN effective_project_assignments epa ON p.id = epa.person_id
    LEFT JOIN scenarios s ON epa.scenario_id = s.id
  ...
  console.log('✅ Updated person_utilization_view to use unified scenario architecture');
}

export async function down(knex: Knex): Promise<void> {
  console.log('Reverting capacity_gaps_view to previous version...');

  // Restore previous views
  await knex.raw('DROP VIEW IF EXISTS person_utilization_view');
  await knex.raw('DROP VIEW IF EXISTS capacity_gaps_view');

  // Restore capacity_gaps_view from migration 030
  await knex.raw(`
    CREATE VIEW capacity_gaps_view AS
    WITH role_capacity AS (
      SELECT 
  ...
  // Restore person_utilization_view from migration 030
  await knex.raw(`
    CREATE VIEW person_utilization_view AS
    SELECT 
      p.id AS person_id,
  ...
  console.log('✅ Reverted to previous capacity_gaps_view');
}