...
async function createTestUser(overrides?: Partial<{ id: string; name: string; email: string }>) {
...
describe('AuditService', () => {
  ...
  describe('getRecentChanges', () => {
    ...
    test('should respect limit and offset parameters', async () => {
      // Create 10 audit entries for different users to avoid auto-cleanup
      const auditPromises = [];
      for (let i = 1; i <= 10; i++) {
        const user = await createTestUser({ id: `user-test-${i}` });
        auditPromises.push(
  ...
  describe('Edge Cases and Corner Cases', () => {
    ...
    test('should handle database connection errors gracefully', async () => {
      ...
      const mockDel = jest.fn().mockRejectedValue(new Error('Database connection failed'));
      const mockWhere = jest.fn();
      
      const invalidDb = jest.fn().mockReturnValue({
        insert: mockInsert,
        count: mockCount,
        where: mockWhere,
        del: mockDel
      ...
      // Make where return itself for chaining
      ...
      const faultyAuditService = new AuditService(invalidDb as any, testConfig);
